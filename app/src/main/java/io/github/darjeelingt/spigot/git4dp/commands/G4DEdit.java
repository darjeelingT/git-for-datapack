...
public class G4DEdit extends BaseCommand {
    public G4DEdit(Main plugin) {
        super(plugin);
    }
    @Override
        ...
        if (args.length == 3) {
            ...
            if (packid == -1) {
                ...
                if (args[1].equals("name")) {
                    sender.sendMessage("Error: Can't Edit Datapack Name");
                } else {
                    if (newPackMap.containsKey(args[1])) {
                        ...
                        try {
                            ...
                            result = true;
                            sender.sendMessage(String.format("Successfully Edited! Datapack \"%s.%s\" = \"%s\"", args[0], args[1], args[2]));
            ...
            sender.sendMessage("Error: Usage: /dpl-edit <datapack_name> <attribute> <value>");
        }
